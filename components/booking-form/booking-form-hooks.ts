    ...
    const loadTripConfig = async () => {
      try {
        ...
        let { data: trip, error: tripError } = await supabase
          .from("trips")
          .select("id,registration_mode,require_pesel,form_show_additional_services,company_participants_info,slug,public_slug,price_cents,payment_split_enabled,payment_split_first_percent,payment_schedule,form_additional_attractions,form_diets,form_extra_insurances,form_required_participant_fields,form_required_contact_fields,seats_total")
          .or(`slug.eq.${slug},public_slug.eq.${slug}`)
          .maybeSingle<TripConfig & { slug: string; public_slug: string | null; price_cents: number | null; payment_split_enabled: boolean | null; payment_split_first_percent: number | null; payment_schedule?: Array<{ installment_number?: number; percent: number }> | null; id: string; seats_total: number | null; form_additional_attractions?: unknown; form_diets?: unknown; form_extra_insurances?: unknown; form_required_participant_fields?: unknown; form_required_contact_fields?: unknown }>();
        ...
        if (trip) {
          ...
          // Zapisz cenę i procent zaliczki (priorytet: payment_schedule)
