-- Naprawa: Dodanie kolumny access_token jeśli nie istnieje
-- Uruchom ten skrypt jeśli kolumna nadal nie istnieje po migracji 015

-- 1. Sprawdź i dodaj kolumnę access_token
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Kolumna access_token została dodana';
  ELSE
    RAISE NOTICE 'Kolumna access_token już istnieje';
  END IF;
END $$;

-- 2. Ustaw tokena dla istniejących rekordów (jeśli są)
UPDATE public.bookings 
SET access_token = gen_random_uuid() 
WHERE access_token IS NULL;

-- 3. Utworzenie indeksu na access_token (jeśli nie istnieje)
...
-- 5. Nadanie uprawnień do funkcji
